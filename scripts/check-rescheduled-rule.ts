...
async function main() {
  ...
  const rules = await prisma.automationRule.findMany({
    where: { workspaceId, name: { contains: 'eschedul', mode: 'insensitive' } },
    select: {
      id: true, name: true, triggerType: true, updatedAt: true,
      steps: {
        orderBy: { order: 'asc' },
        select: {
  ...
  for (const r of rules) {
    console.log(`Rule "${r.name}" id=${r.id} trigger=${r.triggerType} updatedAt=${r.updatedAt.toISOString()}`)
