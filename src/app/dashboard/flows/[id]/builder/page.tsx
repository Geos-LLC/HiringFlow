...
const flowTrace = (topic: string, data?: unknown) => {
  const payload = data !== undefined
    ...
    : ''
  // Local-only console.log so DevTools doesn't mark them red.
          ...
          <FlowSchemaView
            ...
            onStepClick={(stepId) => {
              ...
              if (selectedStepId === stepId) {
                // Clicking the already-active card opens the editor.
                setPopupStepId(stepId)
                setModalPos({ x: 0, y: 0 })
                setCombineEnabled(false)
              } else {
                // First click activates the card so it can be dragged.
